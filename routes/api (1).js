...
const { XeonTwitter } = require('../lib/XeonTwitter');
...
router.get('/primbon/zodiak', async(req, res, next) => {
 ...
 if (text === undefined || apikey === undefined) return res.status(404).send({
    ...
    fetch(encodeURI(`https://www.primbon.com/zodiak/${text}.htm`))
...
router.get('/primbon/artimimpi', async(req, res, next) => {
 ...
 if (text === undefined || apikey === undefined) return res.status(404).send({
    ...
    fetch(encodeURI(`https://www.primbon.com/tafsir_mimpi.php?mimpi=${text}&submit=+Submit+`))
    .then(response => response.json())
        .then(hasil => {
...
router.get('/primbon/artinama', async(req, res, next) => {
  ...
  if (text === undefined || apikey === undefined) return res.status(404).send({
    ...
    fetch(encodeURI(`https://www.primbon.com/arti_nama.php?nama1=${text}&proses=+Submit%21+`))
    .then(response => response.json())
        .then(hasil => {
...
router.get('/download/mediafire', async(req, res, next) => {
 ...
 if (url === undefined || apikey === undefined) return res.status(404).send({
	...
	const baby1 = await mediafireDl(url)

        var result =baby1.link;
             limitAdd(apikey);
                res.json({
                 status : true,
                 result
...
const fg = require('api-dylux')
            let xn = await fg.xnxxdl(query)
        var result = xn.result.files.high;
             limitAdd(apikey);
                res.json({
                 status : true,
...
let restwit = await XeonTwitter(url)
        var result = restwit.url[0].url;
             limitAdd(apikey);
                res.json({
                 status : true,
...
router.get('/downloader/xvideo', async(req, res, next) => {
 ...
 if (query === undefined || apikey === undefined) return res.status(404).send({
    ...
    const fg = require('api-dylux')
	let resx = await fg.xnxxSearch(query)
        var result = resx;
             limitAdd(apikey);
                res.json({
                 status : true,
...
const anutrest = await pinterest(q)
const hasil = anutrest[Math.floor(Math.random() * anutrest.length)]
        var result = hasil;
             limitAdd(apikey);
                res.json({
                 status : true,
...
const { mediafireDl } = require('./lib/mediafire.js')
	const baby11 = await mediafireDl(link)
...
const result = await lyricsv2(judul).catch(async _ => await lyrics(judul))
      limitAdd(apikey);
                res.json({
      status: true,
      code: 200,
...
router.get('/random/faktaunik', fakta);
